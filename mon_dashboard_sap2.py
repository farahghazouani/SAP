...
import numpy as np
import io # Import nécessaire pour st.text(buffer.getvalue())
...
DATA_PATHS = {
    ...
    "performance": r"C:\Users\Farouha\AL_GET_PERFORMANCE_final_cleaned_clean.xlsx", # Nouveau dataset
    "sql_trace_summary": r"C:\Users\Farouha\performance_trace_summary_final_cleaned_clean.xlsx", # Nouveau dataset
    "usr02": r"c:\Users\Farouha\Desktop\fichiers_SAP\usr02_data.xlsx", # Nouveau dataset USR02
}
...
def clean_numeric_with_comma(series):
    ...
    """
    # Convertir en chaîne, supprimer les espaces, et remplacer la virgule décimale par un point
    ...
    try:
        ...
        if file_key == "memory":
            ...
            if 'ENDDATE' in df.columns and 'ENDTIME' in df.columns:
                ...
                # Diagnostic uniquement si FULL_DATETIME est créé but contient des NaN
            ...
            if 'GLTGB' in df.columns:
                # Remplacer '00000000' par NaN pour que pd.to_datetime puisse les gérer
                df['GLTGB'] = df['GLTGB'].astype(str).replace('00000000', np.nan)
                df['GLTGB_DATE'] = pd.to_datetime(df['GLTGB'], format='%Y%m%d', errors='coerce')
            else:
                df['GLTGB_DATE'] = pd.NaT # Créer une colonne NaT si GLTGB est absent
...
kpi_cols = st.columns(5) # 5 colonnes pour 5 KPIs

# KPI 1: Temps de Réponse Moyen Global (Hitlist DB)
avg_resp_time = 0
if not dfs['hitlist_db'].empty and 'RESPTI' in dfs['hitlist_db'].columns:
    avg_resp_time = dfs['hitlist_db']['RESPTI'].mean() / 1000 # Convertir en secondes
...
if not dfs['memory'].empty and 'USEDBYTES' in dfs['memory'].columns:
    avg_memory_usage = dfs['memory']['USEDBYTES'].mean() / (1024 * 1024) # Convertir en Mo
...
kpi_cols[2].metric("Total Appels DB", f"{int(total_db_calls):,}".replace(",", " ")) # Formatage pour lisibilité
...
if not dfs['hitlist_db'].empty and 'CPUTI' in dfs['hitlist_db'].columns:
    avg_cpu_time = dfs['hitlist_db']['CPUTI'].mean() / 1000 # Convertir en secondes
...
tab_titles = [
    ...
    "Analyse des Utilisateurs" # Nouvelle section
]
...
if all(df.empty for df in dfs.values()):
    ...
    # Comme le filtre de date est supprimé, df_hitlist_filtered doit simplement être une copie de l'original
    ...
    if st.session_state.current_section == "Analyse Mémoire":
        ...
        if not df_user.empty:
            ...
            if 'FULL_DATETIME' in df_user.columns and pd.api.types.is_datetime64_any_dtype(df_user['FULL_DATETIME']) and not df_user['FULL_DATETIME'].isnull().all() and 'RESPTI' in df_user.columns and df_user['RESPTI'].sum() > 0:
                ...
                hourly_resp_time = df_user.set_index('FULL_DATETIME')['RESPTI'].resample('H').mean().dropna() / 1000.0 # en secondes
        ...
        if not df_times_data.empty:
            ...
            if all(col in df_times_data.columns for col in perf_cols) and df_times_data[perf_cols].sum().sum() > 0:
                avg_times_by_hour = df_times_data.groupby("TIME")[perf_cols].mean() / 1000.0 # en secondes
        ...
        if not df_task.empty:
            ...
            if 'TASKTYPE' in df_task.columns and all(col in df_task.columns for col in perf_cols_task) and df_task[perf_cols_task].sum().sum() > 0:
                task_perf = df_task.groupby('TASKTYPE')[perf_cols_task].mean().nlargest(10, 'RESPTI') / 1000.0 # en secondes
        ...
        df_hitlist = df_hitlist_filtered.copy() # df_hitlist_filtered est maintenant la copie complète de dfs['hitlist_db']
        if selected_accounts:
        ...
        if not df_hitlist.empty:
            # Le message de plage de dates est maintenu car il est informatif sur les données réelles
        ...
        # Appliquer le filtre WP_TYP si sélectionné
        if selected_wp_types:
        ...
        if not df_sql_trace.empty:
            ...
            if 'SERVERNAME' in df_sql_trace.columns and 'AVGTPERREC' in df_sql_trace.columns:
                # Filtrer spécifiquement pour 'ECC-VE7-00'
                ...
                if not df_ecc_ve7_00.empty and df_ecc_ve7_00['AVGTPERREC'].sum() > 0:
                    # Filtrer les valeurs non nulles et s'assurer qu'il y a plus d'une valeur unique pour la distribution
        ...
        if not df_usr02.empty:
            ...
            if 'GLTGB_DATE' in df_usr02.columns and not df_usr02['GLTGB_DATE'].isnull().all():
                # Filtrer les entrées avec une date de logon valide
                ...
                if not df_valid_logons.empty:
                    ...
                    # Utilisation de px.line avec markers=True pour une meilleure visualisation des points discrets
                    fig_logon_dates = px.line(logon_counts, x='Date de Dernier Logon', y='Nombre d\'Utilisateurs',
                                             ...
                                             color_discrete_sequence=['#6A0DAD']) # Couleur violette pour la ligne
                    
                    # Amélioration de l'axe des x pour un affichage plus clair des dates
                    fig_logon_dates.update_xaxes(
                        # tickformat="%Y-%m-%d", # Format de date plus lisible
                        tickangle=45, # Incliner les étiquettes pour éviter le chevauchement
                        rangeselector=dict(
                            buttons=list([
